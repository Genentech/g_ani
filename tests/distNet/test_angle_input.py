...
from ml_qm.distNet.dist_net import ComputeAngleInput
...
class TestToInput():
    def setup(self):
        ...
        n_features = len(list(self.conf['atom_types'].values())[0])
        confIds = torch.tensor([0,1])
        self.dloader = DataLoader(data_set, confIds, 2)
        self.module = ComputeAngleInput(n_features)
    ...
    def test_triples(self):
        ...
        for batch in self.dloader:
            ...
            for nNeigh, (i_idx, j_idx, dist_ij) in batch_ang_neighbor_map.items():
                ...
                triple_dist_ij, triple_dist_ik, triple_dist_jk \
                   = self.module._computeTriples(self.device_data_set,
                                                 nNeigh, i_idx, j_idx, dist_ij)
    ...
    def test_Ang(self):
        ...
        for batch in self.dloader:
            ...
            batch_ang_input_map = batch['batch_ang_input_map']
            batch_ang_center_map = batch['batch_ang_center_map']

            npt.assert_array_almost_equal(batch_ang_input_map[3][4,5],
                #torch.tensor([ 1.,    1.4141,  0.911,  1.,  0.,  0.,  1.,  0.,0.,  0.,  0., -1.9091, -1.0746,  1.,  0.,  0.,1.,  0.,  0.,  0.,  0., -1.9091, -1.0746,  1.,0.,  0.,  1.,  0.,  0.,  0.,  0., -1.9091,-1.0746]),3)
                torch.tensor([ 1.414, 1. , 0.911, 1. , 0. , 0. , 1. , 0. , 0. , 0. , 0. , -1.909, -1.075, 0.707, 0. , 0. , 0.707, 0. , 0. , 0. , 0. , -1.35 , -0.76 , 1. , 0. , 0. , 1. , 0. , 0. , 0. , 0. , -1.909,-1.075]),3)

            for nNeigh, feature in batch_ang_input_map.items():
                # dimensions are:
                #   nExamples of triple by center atom i
                #   nNeigh * (nNeigh -1): number or triples for this nNeigh
                #   3: ijk
                at_num_i = ds.atoms_long.buffer[batch_ang_center_map[nNeigh],1].numpy()

                for iatom in range(feature.shape[0]):
                    for trpl in range(nNeigh*(nNeigh-1)):
                        feat = feature[iatom,trpl].numpy()
    ...
    def test_Ang_pad(self):
        ...
        module = ComputeAngleInput(n_features)
        ...
        for batch in dloader:
            batch = module(self.device_data_set, batch)
            batch_ang_input_map = batch['batch_ang_input_map']
            batch_ang_center_map = batch['batch_ang_center_map']
            ...
            npt.assert_array_almost_equal(batch_ang_input_map[3][-1,-1],  # a padded example
                torch.tensor([ ds.conf['angleNet']['angularCutoff'],
                              2.24,  0.28,  1.  ,  0.  ,  0.  ,  1.  ,  0.  ,  0.  ,  0.  ,  0.  , -1.91, -1.07,  0.31,  0.  ,  0.  ,  0.31,  0.  ,  0.  , 0.  ,  0.  , -0.6 , -0.34,  0.45,  0.  ,  0.  ,  0.45,  0.  ,  0.  ,  0.  ,  0.  , -0.85, -0.48]), 2)
