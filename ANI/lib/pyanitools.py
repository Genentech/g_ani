...
class anidataloader():
    ...
    ''' Contructor '''
    def __init__(self, store_file, mode="r"):
        if not os.path.exists(store_file):
            exit('Error: file not found - '+store_file)
        self.store = h5py.File(store_file, mode)
    ...
    def h5py_dataset_iterator(self,g, prefix=''):
        ...
        Parameter
        ---------
        g: dict with key being the mol name and items being dict of dataset names
           g = { 'mol003' : { 'species'/ad_C10H20/mol003
        """
        for key in g.keys():
            item = g[key]
